...
public class BolaAmarilla : MonoBehaviour
{

    
    public Vector3 originalBolaAmarillaPosition;
    private Rigidbody rb;

    void Start()
    {
        originalBolaAmarillaPosition = transform.position;
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (GameManager.instance.redBallsCount >= 1)
        {
            if (other.tag == "Hoyo")
            {
                ...
                {
                    SumarPuntos("Amarilla", player);
                    Debug.Log("Metió Camilo la bola amarilla");
                    transform.position = originalBolaAmarillaPosition;
                ...
                {
                    SumarPuntos("Amarilla", player);
                    Debug.Log("Metió Maria del Mar la bola amarilla");
                    transform.position = originalBolaAmarillaPosition;
                    GetComponent<Rigidbody>().velocity = Vector3.zero;
                    GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
