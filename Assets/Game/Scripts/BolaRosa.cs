...
public class BolaRosa : MonoBehaviour
{

   
    public Vector3 originalBolaRosaPosition;
    private Rigidbody rb;

    void Start()
    {
        originalBolaRosaPosition = transform.position;
    ...
    public void OnTriggerEnter(Collider other)
    {
        if (GameManager.instance.redBallsCount >= 1)
        {
            if (other.tag == "Hoyo")
            {
                ...
                {
                    SumarPuntos("Rosa", player);
                    Debug.Log("Metió Camilo la bola rosa");
                    transform.position = originalBolaRosaPosition;
                ...
                {
                    SumarPuntos("Rosa", player);
                    Debug.Log("Metió Maria del Mar la bola rosa");
                    transform.position = originalBolaRosaPosition;
                    GetComponent<Rigidbody>().velocity = Vector3.zero;
                    GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
